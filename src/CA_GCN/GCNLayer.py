...
from src.CA_GCN.graph_util import BottledOrthogonalLinear, log
...
class GraphConvolution(nn.Module):
    def __init__(self, in_features, out_features, edge_types, dropout=0.5, bias=True, use_bn=False,
        ...
        for i in range(int(self.edge_types - 1)):
            self.Gates.append(BottledOrthogonalLinear(in_features=in_features,
                                                      out_features=1,
                                                      bias=bias))
        ...
        self.device = device
        self.to(device) # device = torch.device(device)

    def forward(self, input, adj, rgcn_ith_output=None):
        """
        ...
        ts = []
        if rgcn_ith_output is not None:
            ts.append(rgcn_ith_output)
            ts.append(torch.bmm(adj_[self.edge_types - 1], self.GraphConv[self.edge_types - 1](input)))
        else:
            for i in range(self.edge_types):
                adj_hat_i = adj_[i]
                if i < (self.edge_types - 1):
        ...
        ts = torch.stack(ts)
        ts = ts.sum(dim=0, keepdim=False).to(self.device)
        if self.use_bn:
